...
{
    internal class ServerSystemModLoader : ServerSystem
    {
        #region FIELDS
        private bool started;
        private Jint.JintEngine jintEngine = new Jint.JintEngine();
        private Dictionary<string, string> javascriptScripts = new Dictionary<string, string>();
        private Dictionary<string, IMod> mods = new Dictionary<string, IMod>();
        private Dictionary<string, string[]> modRequirements = new Dictionary<string, string[]>();
        private Dictionary<string, bool> loaded = new Dictionary<string, bool>();
        private IModTypesProvider _modTypesProvider = null;
        #endregion

        #region CONSTRUCTOR
        /// <summary>
        /// Use Constructor only throw IOC! See IOCProvider.DefaultKernel
        /// </summary>
        /// <param name="modTypesProvider"></param>
        public ServerSystemModLoader(IModTypesProvider modTypesProvider)
        {
            jintEngine.DisableSecurity();
            jintEngine.AllowClr = true;
            _modTypesProvider = modTypesProvider;
        }
        #endregion
        ...
        {
            var modTypes = _modTypesProvider.GetTypes();

            foreach (Type t in modTypes)
            {
                if (typeof(IMod).IsAssignableFrom(t))
                {
                    mods[t.Name] = (IMod)_modTypesProvider.GetModAssembly().CreateInstance(t.FullName);
